...
def compute_policy_gradient_loss(
  ...
  response_mask: torch.Tensor | None = None,
    ...
    if importance_reweighting_method == "grpo":
        ...
        return -torch.minimum(unclipped_adv, clipped_adv), {}
    ...
    if importance_reweighting_method == "gspo":
        resample_ratio = torch.exp(
            torch.sum(
                ...
                / torch.sum(response_mask, dim = -1, keepdim=True)).expand_as(old_log_probs)
        unclipped_adv = advantages * resample_ratio
        resample_ratio = torch.clamp(resample_ratio, min=1-cliprange, max=1+cliprange)
        clipped_adv = advantages * resample_ratio
        return -torch.minimum(unclipped_adv, clipped_adv), {}
...
def grpo_train_step(
    ...
    while step < gradient_accumulation_steps:
        ...
        policy_log_probs = get_response_log_probs(model,micro_input_ids,micro_labels)["log_probs"]
        # 计算优势
        ...
        # 计算损失
        per_token_loss,_ = compute_policy_gradient_loss(micro_advantages, policy_log_probs, importance_reweighting_method, micro_old_logprobs, cliprange, micro_response_mask)
